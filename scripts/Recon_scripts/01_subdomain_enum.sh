...
# 8. CRT.sh
##################################
if [ -f "tools/crtsh.py" ]; then
  echo " [+] Running CRT.sh" | tee -a $LOG
  python3 "tools/crtsh.py" -r -d $DOMAIN \
  >> "$OUT/crtsh_$DATE.txt" 2>>"$LOG" || echo "[!] CRT.sh error" >>"$LOG"
else
  echo "[!] CRT.sh not found, skipping" | tee -a "$LOG"
fi
